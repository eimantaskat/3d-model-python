...
def rectangle3D(c,e,RGB): 
  """ c - center, e - width of edges, RGB - color"""
  global vertices, faces
  F = [[0,4,5,1],[0,1,3,2],[0,2,6,4],[1,5,7,3],[2,3,7,6],[4,6,7,5]]
...
def parametric(S,min_u,max_u,grid_u,min_v,max_v,grid_v,RGB,origin): # S - parametric uv surface, grid - detail, RGB - color
  global vertices, faces
...
def sphere(c,r,k,RGB, origin): # c - center, r - radius, k - detail, RGB - color
  ...
  for i in range (k):
    for j in range (k+1):
      ...
      M[i][j] = [x*d,y*d,d]
      vertices += [str(c[0]+M[i][j][0]+origin[0])+' '+str(c[1]+M[i][j][1]+origin[1])+' '+str(c[2]+M[i][j][2]+origin[2])]
  for i in range (k):
    for j in range (k+1):
      vertices += [str(c[0]-M[i][j][2]+origin[0])+' '+str(c[1]+M[i][j][1]+origin[1])+' '+str(c[2]+M[i][j][0]+origin[2])]
  for i in range (k):
    for j in range (k+1):
      vertices += [str(c[0]-M[i][j][0]+origin[0])+' '+str(c[1]+M[i][j][1]+origin[1])+' '+str(c[2]-M[i][j][2]+origin[2])]
  for i in range (k):
  ...
  for i in range (1,k):
    for j in range (1,k):
      vertices += [str(c[0]+M[i][j][0]+origin[0])+' '+str(c[1]+M[i][j][2]+origin[1])+' '+str(c[2]-M[i][j][1]+origin[2])]
  for i in range (1,k):
    for j in range (1,k):
      vertices += [str(c[0]+M[i][j][0]+origin[0])+' '+str(c[1]-M[i][j][2]+origin[1])+' '+str(c[2]+M[i][j][1]+origin[2])]
